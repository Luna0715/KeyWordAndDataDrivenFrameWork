...
def TestSendMailAndCreateContacts():
    try:
        ...
        for idx,i in enumerate(isExecuteColumn[1:]):
            ...
            if i.value.lower() == "y":
                ...
                if useFrameWorkName == "数据":
                    logging.info("******调用数据驱动******")
                    #获取测试用例表中，第idx+1行，执行框架为数据驱动的用例所使用的的数据sheet名
                    ...
                    for index in range(2,stepNums +1):
                        #因为第一行是标题行，无需执行，获取步骤sheet中第index行对象
                        ...
                        try:
                            ...
                            logging.info("执行步骤 '%s' 发生异常") %stepRow[testStep_testStepDescribe -1].value
                            #截取异常屏幕图片
        ...
        logging.info("共 %d 条用例，%d 条需要被执行，成功执行 %d 条" %(len(isExecuteColumn)-1,requireCase,successfulCase))
    except Exception as e:
